...
const mockReadFileSync = vi.mocked(nodefs.readFileSync)
const mockReaddirSync = vi.mocked(nodefs.readdirSync)
...
describe('runTests', () => {
  ...
  it('returns exitCode 1 and parses failing test names', async () => {
    ...
    mockExeca.mockRejectedValueOnce(
      Object.assign(new Error('exit 1'), { exitCode: 1, stdout: fakeOutput, stderr: '' })
    )
...
describe('runTypecheck', () => {
  ...
  it('runs tsc --noEmit when tsconfig.json exists and returns exit code', async () => {
    ...
    const result = await runTypecheck('/fake/worktree')
    expect(result.exitCode).toBe(0)
    expect(mockExeca).toHaveBeenCalledWith('tsc', ['--noEmit'], expect.objectContaining({ cwd: '/fake/worktree' }))
  })

  it('returns exitCode 1 when tsc fails', async () => {
    mockExistsSync.mockReturnValueOnce(true)
    mockExeca.mockRejectedValueOnce(
      Object.assign(new Error('tsc error'), { exitCode: 1, stdout: 'error TS2322', stderr: '' })
    )
...
describe('artifactExists', () => {
  ...
  beforeEach(() => {
    ...
    tmpDir = fs.mkdtempSync(join(os.tmpdir(), 'pipe3-test-'))
  })
  ...
  it('returns true when file exists', () => {
    fs.writeFileSync(join(tmpDir, 'output.json'), '{}')
...
describe('runStyleGates', () => {
  ...
  beforeEach(() => {
    mockExistsSync.mockImplementation((p: unknown) => String(p).includes('src'))
  })
  ...
  it('detects style={{ inline style violation', async () => {
    ...
    mockReadFileSync.mockImplementation((p: unknown) => {
      if (String(p).endsWith('App.tsx')) return '<div style={{ color: "red" }}>'
      return ''
    })
  ...
  it('detects console.log in src files', async () => {
    ...
    mockReadFileSync.mockImplementation((p: unknown) => {
      if (String(p).endsWith('utils.ts')) return 'console.log("debug")'
      return ''
    })
    mockExistsSync.mockReturnValue(true)
  ...
  it('detects arbitrary Tailwind class values', async () => {
    ...
    mockReadFileSync.mockImplementation((p: unknown) => {
      if (String(p).endsWith('Button.tsx')) return 'className="text-[14px] font-bold"'
      return ''
    })
    mockExistsSync.mockReturnValue(true)
