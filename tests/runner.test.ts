...
describe('spawnAgent — claude harness', () => {
  it('spawns claude with --print -p prompt --cwd worktreePath (no contextFile)', async () => {
    ...
    const result = await spawnAgent('claude', 'researcher', 'do the thing', null, '/tmp/wt')

    expect(mockExeca).toHaveBeenCalledWith(
      'claude',
      ['--print', '-p', 'do the thing', '--cwd', '/tmp/wt']
    )
...
describe('spawnAgent — codex harness', () => {
  it('spawns codex exec --json prompt -C worktreePath with empty stdin when no contextFile', async () => {
    ...
    const result = await spawnAgent('codex', 'test-writer', 'write tests', null, '/tmp/wt')
...
describe('spawnAgent — opencode harness', () => {
  it('spawns opencode run --format json --dir worktreePath prompt (no contextFile)', async () => {
    ...
    const result = await spawnAgent('opencode', 'verifier', 'verify things', null, '/tmp/wt')

    expect(mockExeca).toHaveBeenCalledWith(
      'opencode',
      ['run', '--format', 'json', '--dir', '/tmp/wt', 'verify things']
    )
  ...
  it('appends --file contextFile when contextFile provided', async () => {
    ...
    await spawnAgent('opencode', 'verifier', 'verify things', '/tmp/ctx.md', '/tmp/wt')

    expect(mockExeca).toHaveBeenCalledWith(
      'opencode',
      ['run', '--format', 'json', '--dir', '/tmp/wt', 'verify things', '--file', '/tmp/ctx.md']
    )
  })
})
...
describe('spawnAgent — pi harness', () => {
  it('spawns pi --mode rpc --no-session with cwd and stdin:pipe, sends JSONL, reads until agent_end', async () => {
    ...
    const subprocess: any = Object.assign(
    ...
    const result = await spawnAgent('pi', 'researcher', 'research this', '/tmp/ctx.md', '/tmp/wt')
