...
function codexAgentSkillConfig(
  config: PipelineConfig,
  profile: PipelineConfig["profiles"][string],
  cwd: string
  ...
  const skillConfig = (profile.skills ?? []).flatMap((id) => {
    ...
    const absoluteSkillPath = skillPath ? join(cwd, skillPath) : undefined;
...
function codexAgentMcpConfig(
  ...
  const mcpServers = Object.fromEntries(
    (profile.mcp_servers ?? []).flatMap((id) => {
      ...
      return server ? [[id, codexAgentMcpServerConfig(server)] as const] : [];
    })
  );
  return Object.keys(mcpServers).length > 0 ? { mcp_servers: mcpServers } : {};
}

function codexAgentMcpServerConfig(
